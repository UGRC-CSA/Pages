...
function answerLocally(question){
  ...
  // definition.
  // A greeting is not a question. Answer it instead of shrugging at it.
  if (/^\s*(yo|hi|hey|hello|sup|hiya|howdy)\b[\s!.?]*$/i.test(question)) {
    return 'Hello. Ask me about anything on this page. Try <b>what is contrast</b>, '
      + '<b>what is padding</b>, <b>what is a selector</b>, or the name of any token, '
      + 'like <b>what is --ocs-accent</b>.';
  }
  ...
  for (const item of KB) {
    if (!item.a) continue;
    const iwList = words(item.q);
    const iw = new Set(iwList);
    let score = 0, exact = 0, partial = 0;
    qw.forEach(w => {
      if (iw.has(w)) { score += item.w; exact++; return; }
      // Part-words, so a half-typed "contra" still finds "contrast".
      if (w.length >= 4 && iwList.some(k => k.startsWith(w) || w.startsWith(k))) {
        score += item.w; partial++;
      }
    });
    // A part-word on its own is only trustworthy when it IS the question.
    // Otherwise "why would a designer pick a pill shape" matches the entry for
    // "design token" on one stem and answers something nobody asked.
    if (!exact && partial && qw.length > 1) continue;
...
async function askGroq(question){
  ...
  try {
    const r = await fetch('https://api.groq.com/openai/v1/chat/completions', {
      ...
      // gpt-oss models think before they answer, and that thinking spends the
      // token budget. At 220 tokens the reply came back empty every time.
      // reasoning_effort 'low' keeps it short; the headroom covers the rest.
      body: JSON.stringify({model: 'openai/gpt-oss-20b', max_tokens: 900,
        temperature: 0.3, reasoning_effort: 'low',
        messages: [{role: 'system', content: sys},
                   {role: 'user', content: ctx + '\n\nQuestion: ' + question}]})
    });
    if (r.status === 401 || r.status === 403) {
      return 'That key was refused. Check it under Settings below, or make a new one at console.groq.com/keys.';
    }
    if (r.status === 429) return 'Groq is rate limiting that key. Wait a few seconds and ask again.';
    if (!r.ok) return `Groq answered with HTTP ${r.status}. Try again in a moment.`;
    const j = await r.json();
    const raw = (j.choices?.[0]?.message?.content || '').trim();
    if (!raw) return 'The model replied with nothing that time. Ask again, or word it differently.';
    // Escape first, then allow the little markdown the model actually emits.
...
async function coachAsk(){
  ...
  coachThread.pop();
  pushAsk('coach', g || (
    'I could not match that to anything on this page. I can answer:<br>'
    + '\u2022 any token by name, like <b>what is --ocs-accent</b><br>'
    + '\u2022 CSS words: <b>selector</b>, <b>property</b>, <b>padding</b>, <b>margin</b>, '
    + '<b>hover</b>, <b>nesting</b>, <b>contrast</b>, <b>px</b> and <b>rem</b><br>'
    + '\u2022 <b>what do I do</b> for the next step, or <b>why is it not working</b> '
    + 'to read the checks<br>'
    + 'For anything else, add your own Groq key under Settings below and I will ask it.'));
}
